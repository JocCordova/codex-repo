...
def main() -> int:
    ...
    scored = score_items(new_items, topics, float(run_cfg["min_relevance_score"]))

    cluster_cfg = config["clustering"]
    clusters = cluster_items(scored, int(cluster_cfg["max_clusters"]), int(cluster_cfg["min_cluster_size"]))
    selected = select_top_clusters(clusters, int(run_cfg["top_clusters"]))

    tz = ZoneInfo(run_cfg["timezone"])
    ...
    send_digest(selected, config["whatsapp"])
